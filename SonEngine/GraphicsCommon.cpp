...
void Graphics::InitializeCommonState(const Microsoft::WRL::ComPtr<ID3D12Device5>& device)
{
	...
	g_C1_RS.Reset(1, 0);
	g_C1_RS[0].InitCBV(0);
	g_C1_RS.Finalize(device, L"C1_RS", D3D12_ROOT_SIGNATURE_FLAG_ALLOW_INPUT_ASSEMBLER_INPUT_LAYOUT);
	...
	g_R1_C3_RS.Reset(4, 1);
	g_R1_C3_RS[0].InitAsDescriptorRange(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 0, 6); // albedo
	g_R1_C3_RS[1].InitCBV(0); // local
	g_R1_C3_RS[2].InitCBV(1); // global
	g_R1_C3_RS[3].InitCBV(2); // skinned transforms
	g_R1_C3_RS.InitStaticSampler(0, wrapLinearSampler);
	g_R1_C3_RS.Finalize(device, L"R1_C3_RS", D3D12_ROOT_SIGNATURE_FLAG_ALLOW_INPUT_ASSEMBLER_INPUT_LAYOUT);


	g_R2_C2_RS.Reset(4, 1);
	g_R2_C2_RS[0].InitAsDescriptorRange(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 0, 3); // cubemap
	g_R2_C2_RS[1].InitAsDescriptorRange(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 3, 6); // albedo
	...
	g_R4_C3_RS.Reset(7, 2);
	g_R4_C3_RS[0].InitAsDescriptorRange(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 0, 4); // cubemap
	g_R4_C3_RS[1].InitAsDescriptorRange(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 4, 6); // albedo
	g_R4_C3_RS[2].InitAsDescriptorRange(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 10, 1); // "반사"용 cubemap
	g_R4_C3_RS[3].InitAsDescriptorRange(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 11, 1); // depth only
	g_R4_C3_RS[4].InitCBV(0); // local
	g_R4_C3_RS[5].InitCBV(1); // global
	g_R4_C3_RS[6].InitCBV(2); // skinned
	g_R4_C3_RS.InitStaticSampler(0, wrapLinearSampler);
	g_R4_C3_RS.InitStaticSampler(1, clampLinearSampler);
	g_R4_C3_RS.Finalize(device, L"R4_C3_RS", D3D12_ROOT_SIGNATURE_FLAG_ALLOW_INPUT_ASSEMBLER_INPUT_LAYOUT);
