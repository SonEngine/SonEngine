...
template<typename V, typename I>
inline aiMatrix4x4 ModelLoader<V, I>::MatrixToAi(const DirectX::SimpleMath::Matrix& m)
{
	return aiMatrix4x4(
...
template<typename V, typename I>
inline DirectX::SimpleMath::Matrix ModelLoader<V, I>::aiToMatrix(const aiMatrix4x4& m)
{
...
template<typename V, typename I>
const aiNode* ModelLoader<V, I>::FindParent(Asset<V, I>& asset, const aiNode* node) {
	if (!node)
		return nullptr;
...
template<typename V, typename I>
inline void ModelLoader<V, I>::ProcessNode(Asset<V, I>& asset, aiNode* node, const aiScene* scene, DirectX::SimpleMath::Matrix tr, bool loadAnimation)
{
	auto& boneNameToId = asset.animData.boneNameToId;
	if (node->mParent &&
		boneNameToId.count(node->mName.C_Str()))
	{
		auto parentsNode = FindParent(asset, node->mParent);
		if (parentsNode != nullptr)
		{
			const auto boneId = boneNameToId[node->mName.C_Str()];
			asset.animData.boneParents[boneId] =
				boneNameToId[parentsNode->mName.C_Str()];
		}
	}
...
template<typename V, typename I>
inline void ModelLoader<V, I>::GetBonePosition(Asset<V, I>& asset, aiNode* node, const aiMatrix4x4& parentTransform, const aiScene* scene)
{
