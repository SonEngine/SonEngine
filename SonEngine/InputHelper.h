...
namespace Input {
	...
	public:
		...
		bool GetInteractState() { return InputStates[interactKey]; };
	
		DirectX::SimpleMath::Vector3 ExecuteCommands(const float& deltaTime, const Actor* actor);
		std::pair<Vector3, bool> ExecutePlayerCommands(const float& deltaTime, const Actor* actor);
	...
	public:
		...
		bool interactDirty = false;

	public:
		size_t interactKey = size_t('E');
		...
		size_t grabKey = size_t('G');
		size_t jumpKey = size_t(' ');
	};
};
